...
import net.whg.we.command.DynamicVariableArgument;
import net.whg.we.command.StringArgument;
import net.whg.we.command.VariableArgument;
...
public class ForCommand implements CommandHandler
{
	...
	public String getHelpText()
	{
		...
		sb.append("  For each value between <min> (inclusive) and max (exclusive), assigns <var>\n"
		...
		sb.append("  For each line in the <list>, assigns <var> to that value and then executes\n"
	...
	public String executeCommand(Command command)
	{
		...
		if (args.length == 5)
		{
			...
			if (!(args[3] instanceof StringArgument) && !args[3].getValue().equals("do"))
			{
				console.println("Parameter 'do' must be placed before command!");
				return "";
			}
		...
		if (args.length == 6)
		{
			...
			if (!(args[4] instanceof StringArgument) && !args[4].getValue().equals("do"))
			{
				console.println("Parameter 'do' must be placed before command!");
				return "";
			}
		...
		if (args.length == 4)
		{
			...
			if (!(args[2] instanceof StringArgument) && !args[2].getValue().equals("do"))
			{
				console.println("Parameter 'do' must be placed before command!");
				return "";
			}
