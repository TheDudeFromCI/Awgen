...
public class FirstPersonCamera extends PersonCamera
{
    ...
    public void updateCameraRotation()
    {
        ...
        float yaw = Input.getDeltaMouseX() * Time.deltaTime()
                * getMouseSensitivity();
        float pitch = Input.getDeltaMouseY() * Time.deltaTime()
                * getMouseSensitivity();

        _rotationBuffer.x =
                MathUtils.clamp(_baseRotation.x - pitch, -MAX_ANGLE, MAX_ANGLE);
    ...
    public void updateCameraPosition()
    {
        ...
        Vector3f backward = _camera.getLocation().getInverseMatrix()
                .positiveZ(new Vector3f());
        Vector3f right = _camera.getLocation().getInverseMatrix()
                .positiveX(new Vector3f());
