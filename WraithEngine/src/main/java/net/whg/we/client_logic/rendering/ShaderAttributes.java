...
public class ShaderAttributes implements Externalizable
{
	...
	/**
	 * Creates a new, empty, shader attrbutes object. This uses the initial
	 * internal buffer size of 4. Buffers are resized as needed.
	 */
	 ...
	 *
	 * @param bufferSize
	 *     - The initial size of the internal arrays. This is automatically
	 *     resized as needed.
	 */
	 ...
	 * arraylist does not have the capacity to hold a new attribute, the buffer
	 * is increased by 4 slots.
	 *
	 * @param name
	 *     - The name of the new attribute.
	 * @param size
	 *     - The size of the new attribute. This must be a value between 1 and
	 *     4, inclusive.
	 */
	public void addAttribute(String name, int size)
	{
		if (size <= 0)
			throw new IllegalArgumentException(
					"Attribute sizes must be a positive integer!");
		if (size > 4)
			throw new IllegalArgumentException(
					"Attribute sizes must 4 or lower!");
	...
	/**
	 * Removes the attribute at the given index. All attribute after this are
	 * moved down.
	 *
	 * @param index
	 ...
	 * @return The size of the attribute, or -1 if the attribute could not be
	 *     found.
	 */
	...
	/**
	 * Gets the index of the attribute with the given name. Preforms a search
	 * and returns the index of the first attribute found with a matching name.
	 *
	 * @param attrib
	 *     - The name of the attribute.
	...
	/**
	 * Gets the starting position of this attribute within the vertex, based on
	 * the sizes of all previous attributes.
	 *
